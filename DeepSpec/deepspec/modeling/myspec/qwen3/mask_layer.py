...
class Qwen3MySpecMaskAttention(nn.Module):
    def __init__(self, config, layer_idx: int, num_latent_layers: int):
        super().__init__()
        self.config = config
        ...
        self.sliding_window = (
            ...
            if config.layer_types[layer_idx - self.num_latent_layers] == "sliding_attention"
            else None
        )
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        v = torch.cat([v_ctx, v_latent, v_noise], dim=1).view(
            bsz, ctx_len + latent_len + q_len, self.num_key_value_heads, self.head_dim
        )
        ...
        attn_fn: Callable = (eager_attention_forward if attn_impl == "eager" else ALL_ATTENTION_FUNCTIONS[attn_impl])
...
class Qwen3MySpecMaskDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config, layer_idx: int, num_latent_layers: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = Qwen3MySpecMaskAttention(config=config, layer_idx=layer_idx, num_latent_layers=num_latent_layers)
